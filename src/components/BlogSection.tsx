...
export const BlogSection: React.FC<BlogSectionProps> = ({ onSelectPost }) => {
  ...
  return (
    <section className="py-20 sm:py-28 bg-[#0E1430] text-white relative border-y border-slate-800/80 overflow-hidden" id="blog-section">
        ...
        >
          ...
          <div className="mt-8 max-w-md mx-auto relative">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-3 rounded-xl bg-slate-900 border border-slate-700 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-cyan-400 focus:ring-1 focus:ring-cyan-400 transition-colors"
            />
          </div>
          ...
          <AnimatePresence>
            {filteredPosts.map((post, idx) => (
              <motion.article
                ...
                whileHover={{ y: -8, borderColor: 'rgba(37, 99, 235, 0.6)' }}
                className="rounded-2xl bg-[#111836] border border-slate-800/90 overflow-hidden flex flex-col justify-between transition-all duration-300 shadow-xl group"
              >
                <div>
                ...
                <div className="p-6 pt-0">
                  <motion.button
                    ...
                    className="w-full py-2.5 rounded-xl bg-slate-900 hover:bg-blue-600 text-slate-200 hover:text-white text-xs sm:text-sm font-semibold border border-slate-700/80 transition-all flex items-center justify-center gap-2 group/btn cursor-pointer"
                  >
