  ...
  return (
    <section className="py-20 sm:py-28 bg-[#0B1026] relative" id="services-section">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {[
              ...
              return (
                <button
                  ...
                  className={`relative px-4 py-2 rounded-xl text-xs sm:text-sm font-semibold transition-colors z-10 cursor-pointer ${
                    active ? 'text-white' : 'text-slate-400 hover:text-white bg-slate-900/70 border border-slate-800'
                  }`}
                >
                  {active && (
                    <motion.div
                      layoutId="activeServiceTab"
                      className="absolute inset-0 bg-blue-600 rounded-xl shadow-lg shadow-blue-600/30 -z-10"
        ...
        {/* 9 Services Grid with Motion Layout */}
          ...
          <AnimatePresence>
            {filteredServices.map((service, index) => (
              <motion.div
                ...
                whileHover={{ y: -6, borderColor: 'rgba(56, 189, 248, 0.45)' }}
                className="relative rounded-2xl bg-[#0F1635] border border-slate-800/80 p-6 sm:p-7 flex flex-col justify-between transition-colors shadow-lg hover:shadow-2xl hover:shadow-blue-950/60 group"
              >
                <div>
                  {/* Top Badge & Icon */}
                ...
                <div className="pt-4 border-t border-slate-800/80 flex items-center justify-between gap-3">
                  <button
                    ...
                    className="text-xs sm:text-sm font-semibold text-cyan-400 hover:text-cyan-300 flex items-center gap-1.5 group/link transition-colors cursor-pointer"
                  ...
                  <button
                    ...
                    className="text-xs font-semibold px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 hover:text-white transition-colors cursor-pointer"
                  >
                    Consult Now
                  </button>
                </div>
        ...
        >
          ...
          <motion.button
            ...
            className="px-8 py-3.5 rounded-xl brand-gradient text-white font-semibold text-sm shadow-xl shadow-blue-600/30 hover:shadow-cyan-500/30 transition-all inline-flex items-center gap-2 cursor-pointer"
          >
